...
} from '@/lib/ai/personal-statement-generator';
import {
...
export default function PersonalStatementPage() {
  ...
  const [result, setResult] = useState<PersonalStatementOutput | null>(null);
  ...
  const [academicInterest] = useState('');
  const [academicBackground, setAcademicBackground] = useState('');
  const [researchExperience, setResearchExperience] = useState('');
  const [careerGoals, setCareerGoals] = useState('');
  const [whyThisProgram, setWhyThisProgram] = useState('');
  ...
  const handleGenerate = async () => {
    ...
    const input: PersonalStatementInput = {
      ...
      contentMaterials: {
        backgroundStory,
        academicInterest: academicInterest || currentMajor,
        academicBackground,
        researchExperience,
        careerGoals,
        whyThisProgram,
    ...
    try {
      const response = await fetch('/api/ai/personal-statement/generate', {
        method: 'POST',
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <AIProductIntro product="ps" />
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">AI 个人陈述 PS 参考草稿</h1>
        <p className="mt-2 text-gray-600">
          生成完整英文 Personal Statement 成稿，供您参考和修改
        </p>
      ...
      <div className="space-y-8">
        ...
        {/* PS内容素材 */}
        <section className="bg-white rounded-xl border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">PS 内容素材</h2>
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">背景故事（您为什么选择这个专业方向）</label>
              <textarea
                value={backgroundStory}
                onChange={(e) => setBackgroundStory(e.target.value)}
                placeholder="描述您的学术兴趣起源和背景经历..."
                rows={4}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">研究经历（相关科研或项目经验）</label>
              <textarea
                value={researchExperience}
                onChange={(e) => setResearchExperience(e.target.value)}
                placeholder="描述您参与的研究项目或学术经历..."
                rows={4}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">学术背景（相关课程、研究、培训）</label>
              <textarea
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">关键成就（学术或专业方面的主要成就）</label>
              <textarea
                value={keyAchievements}
                onChange={(e) => setKeyAchievements(e.target.value)}
                placeholder="描述您最重要的学术成就或荣誉..."
                rows={3}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">成长与挑战（经历过的困难和从中获得的成长）</label>
              <textarea
                value={challenges}
                onChange={(e) => setChallenges(e.target.value)}
                placeholder="分享一次挑战经历和您的反思..."
                rows={3}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">职业目标（长期职业发展规划）</label>
              <textarea
                value={careerGoals}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">为什么选择这个项目</label>
              <textarea
                value={whyThisProgram}
                onChange={(e) => setWhyThisProgram(e.target.value)}
                placeholder="说明为什么您选择这个项目以及它如何符合您的目标..."
                rows={3}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">PS 语气</label>
